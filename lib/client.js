...
class Client extends EventEmitter {
  constructor (options) {
    ...
    this.log = options.logger || require('./logger')({level: options.logLevel, logger: options.logger});
  ...
  send ({host, port = 9999, payload, timeout = this.timeout, debug = false}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.on('connect', () => {
        this.log.debug('client.send: socket: connect');
        socket.write(encryptWithHeader(payloadString));
      });

      socket.on('data', (data) => {
        this.log.debug('client.send: socket: data');
        deviceData += decrypt(data.slice(4)).toString('ascii');
        socket.end();
      });

      socket.on('end', () => {
        this.log.debug('client.send: socket: end');
        socket.destroy();
        let data;
        try {
        ...
        if (!data.err_code || data.err_code === 0) {
          ...
          let errMsg = data;
          this.log.error('TPLink Device response error %j' + data);
      ...
      socket.on('error', (err) => {
        this.log.error('TPLink Device TCP error');
        this.log.trace(err);
        socket.destroy();
        reject(err);
      });
  ...
  startDiscovery ({address, port, broadcast = '255.255.255.255', discoveryInterval = 10000, discoveryTimeout = 0, offlineTolerance = 3, deviceTypes, devices} = {}) {
    ...
    this.socket.on('error', (err) => {
      ...
      this.log.error('client.startDiscovery: UDP error');
      this.log.trace(err);
      this.socket.close();
      this.emit('error', err);
    });
    ...
    this.socket.on('message', (msg, rinfo) => {
      ...
      this.log.debug('client.startDiscovery: message: ' + decryptedMsg.toString('ascii') + ' from ' + rinfo.address);

      let parsedMsg;
      try {
        parsedMsg = JSON.parse(decryptedMsg);
      } catch (e) {
        this.log.error('client.startDiscovery: Error parsing JSON - From: [%s] Original: [%s] Decrypted: [%s]', msg, decryptedMsg, rinfo.address);
        return;
      }
