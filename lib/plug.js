...
class Plug extends EventEmitter {
  ...
  emitEvents () {
    ...
    if (this.debug) { console.log('DEBUG: emitEvents() inUse: %s relayOn: %s lastState: %j', inUse, relayOn, this.lastState); }
  ...
  startPolling (interval, event) {
    this.pollingTimer = setInterval(() => {
      var info = this.getInfo();
      if (event !== undefined &&  typeof event === "function") { info.then(event); }
    }, interval);
    return this;
  }
